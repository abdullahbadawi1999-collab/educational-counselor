...
module.exports = function(sql) {
  router.get('/', async (req, res) => {
    ...
    try {
      ...
      if (circle_id && search) {
        students = await sql`SELECT s.id, s.name, s.student_phone, s.parent_phone_1, s.parent_phone_2, s.notes, s.circle_id, c.name as circle_name, c.teacher_name FROM students s JOIN circles c ON s.circle_id = c.id WHERE s.circle_id = ${circle_id} AND s.name LIKE ${'%' + search + '%'} ORDER BY s.name LIMIT ${parseInt(limit)} OFFSET ${offset}`;
      } else if (circle_id) {
        students = await sql`SELECT s.id, s.name, s.student_phone, s.parent_phone_1, s.parent_phone_2, s.notes, s.circle_id, c.name as circle_name, c.teacher_name FROM students s JOIN circles c ON s.circle_id = c.id WHERE s.circle_id = ${circle_id} ORDER BY s.name LIMIT ${parseInt(limit)} OFFSET ${offset}`;
      } else if (search) {
        students = await sql`SELECT s.id, s.name, s.student_phone, s.parent_phone_1, s.parent_phone_2, s.notes, s.circle_id, c.name as circle_name, c.teacher_name FROM students s JOIN circles c ON s.circle_id = c.id WHERE s.name LIKE ${'%' + search + '%'} ORDER BY s.name LIMIT ${parseInt(limit)} OFFSET ${offset}`;
      } else {
        students = await sql`SELECT s.id, s.name, s.student_phone, s.parent_phone_1, s.parent_phone_2, s.notes, s.circle_id, c.name as circle_name, c.teacher_name FROM students s JOIN circles c ON s.circle_id = c.id ORDER BY s.name LIMIT ${parseInt(limit)} OFFSET ${offset}`;
      }
