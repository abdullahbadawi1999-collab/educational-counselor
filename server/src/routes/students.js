...
module.exports = function(sql) {
  router.get('/', async (req, res) => {
    ...
    try {
      ...
      if (circle_id && search) {
        students = await sql`SELECT s.id, s.name, s.student_phone, s.parent_phone_1, s.parent_phone_2, s.notes, s.circle_id, c.name as circle_name, c.teacher_name, (SELECT COUNT(*)::int FROM behaviors b WHERE b.student_id = s.id AND b.type = 'negative') as negative_count, (SELECT COUNT(*)::int FROM alerts a WHERE a.student_id = s.id AND a.status = 'pending') as pending_alerts, (SELECT STRING_AGG(DISTINCT COALESCE(bt.name, b.description), '، ') FROM behaviors b LEFT JOIN behavior_types bt ON b.behavior_type_id = bt.id WHERE b.student_id = s.id AND b.type = 'negative' AND NOT EXISTS (SELECT 1 FROM actions a WHERE a.behavior_id = b.id)) as pending_violations FROM students s JOIN circles c ON s.circle_id = c.id WHERE s.circle_id = ${circle_id} AND s.name LIKE ${'%' + search + '%'} ORDER BY s.name LIMIT ${parseInt(limit)} OFFSET ${offset}`;
      } else if (circle_id) {
  ...
  router.get('/:id/behaviors', async (req, res) => {
    try {
      const behaviors = await sql`
        ...
        WHERE b.student_id = ${req.params.id} AND b.type = 'negative'
